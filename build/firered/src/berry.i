...
void InitEnigmaBerry(void)
{
    ...
    gSaveBlock1Ptr->enigmaBerry.berry = ({ const struct Berry2 * berries = (const struct Berry2 *)gBerries; berries[175 - 133]; });
    for (i = 0; i < 18; i++)
...
{
    if (berryIdx == (((175 - 133) + 1)) && IsEnigmaBerryValid())
        return (struct Berry *)&gSaveBlock1Ptr->enigmaBerry.berry;

    if (berryIdx == 0 || berryIdx > (((175 - 133) + 1)))
        berryIdx = 1;
...
{
    if (itemId - 133 < 0 || itemId - 133 > 175 - 133)
        return 1;
...
{
    if (berryType - 1 < 0 || berryType - 1 > 175 - 133)
        return 133;

    return berryType + 133 - 1;
}
