...
static void Task_RunTimeBasedEvents(u8 taskId)
{
    ...
    if (!ArePlayerFieldControlsLocked() && !(gQuestLogState == 2 || gQuestLogState == 3))
...
{
    u8 i;
    for (i = 0; i < (sizeof(sIcefallCaveIceCoords) / sizeof((sIcefallCaveIceCoords)[0])); i++)
    {
        if (sIcefallCaveIceCoords[i][0] + 7 == x && sIcefallCaveIceCoords[i][1] + 7 == y)
        {
            FlagSet(i + 1);
            break;
        }
...
void SetIcefallCaveCrackedIceMetatiles(void)
{
    ...
    for (i = 0; i < (sizeof(sIcefallCaveIceCoords) / sizeof((sIcefallCaveIceCoords)[0])); i++)
    {
        if (FlagGet(i + 1) == 1)
        {
            int x = sIcefallCaveIceCoords[i][0] + 7;
            int y = sIcefallCaveIceCoords[i][1] + 7;
            MapGridSetMetatileIdAt(x, y, 0x35A);
        }
    }
}
# 173 "src/field_tasks.c"
static void IcefallCaveIcePerStepCallback(u8 taskId)
{
    s16 x, y;
    u8 tileBehavior;
