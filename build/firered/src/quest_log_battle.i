    ...
    if (!(gBattleTypeFlags & ((1 << 1) | (1 << 9) | (1 << 16))) && (gBattleOutcome == 1 || gBattleOutcome == 7))
    {
        ...
        if (gBattleTypeFlags & (1 << 3))
        {
            ...
            if (gBattleTypeFlags & (1 << 0))
            {
                ...
                if (GetBattlerSide(gBattleStruct->lastAttackerToFaintOpponent) == 0)
        ...
        else
        {
            if (gBattleOutcome == 1)
            {
                wildData->defeatedSpecies = GetMonData(gEnemyParty, 11);
                wildData->caughtSpecies = 0;
            }
            else
            {
                wildData->defeatedSpecies = 0;
                wildData->caughtSpecies = GetMonData(gEnemyParty, 11);
            }
            wildData->mapSec = GetCurrentRegionMapSectionId();
            SetQuestLogEvent(31, (const u16 *)wildData);
        }
        Free(trainerData);
        Free(wildData);
    }
