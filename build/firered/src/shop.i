...
static __attribute__((section("ewram_data"))) struct ShopData sShopData = {0};
...
__attribute__((section("ewram_data"))) struct QuestLogEvent_Shop sHistory[2] = {0};
...
static u8 GetMartTypeFromItemList(u32 martType)
{
    ...
    {
        if (ItemId_GetPocket(sShopData.itemList[i]) == 4)
...
bool8 BuyMenuBuildListMenuTemplate(void)
{
    ...
    if (sShopMenuListMenu == ((void *)0)
     || (sShopMenuItemStrings = Alloc((sShopData.itemCount + 1) * sizeof(*sShopMenuItemStrings))) == ((void *)0))
    {
        BuyMenuFreeMemory();
        SetShopExitCallback();
        return 0;
...
static void BuyMenuPrintItemDescriptionAndShowItemIcon(s32 item, bool8 onInit, struct ListMenu *list)
{
    ...
    if (sShopData.martType != MART_TYPE_TMHM)
    {
        ...
        if (item != -2)
            ...
            CreateItemMenuIcon(375, sShopData.itemSlot);
...
static void LoadTmHmNameInMart(s32 item)
{
    if (item != -2)
    {
        ConvertIntToDecimalStringN(gStringVar1, item - 288, 2, 2);
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    if (AdjustQuantityAccordingToDPadInput(&data[1], sShopData.maxQuantity) == 1)
    {
        sShopData.itemPrice = ItemId_GetPrice(data[5]) * data[1];
        BuyMenuPrintItemQuantityAndPrice(taskId);
    }
    else
    {
...
static void BuyMenuTryMakePurchase(u8 taskId)
{
    ...
    if (AddBagItem(data[5], data[1]) == 1)
    {
        ...
        RecordItemTransaction(data[5], data[1], 37 - (37 - 1));
    }
    else
    {
...
void RecordItemTransaction(u16 itemId, u16 quantity, u8 logEventId)
{
    ...
    if (history->lastItemId != 0)
        history->hasMultipleTransactions = 1;
...
static void RecordTransactionForQuestLog(void)
{
    ...
    if (eventId != 0)
        SetQuestLogEvent(eventId + (37 - 1), (const u16 *)&sHistory[0]);

    eventId = sHistory[1].logEventId;
    if (eventId != 0)
        SetQuestLogEvent(eventId + (37 - 1), (const u16 *)&sHistory[1]);
}
